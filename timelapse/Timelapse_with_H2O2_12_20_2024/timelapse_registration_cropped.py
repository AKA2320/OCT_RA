...
def run_scans(scan_num):
    ...
    pics_without_line = pics_without_line.astype(np.float32)[:,np.r_[50:150,490:540]]
    global_min, global_max = np.min(pics_without_line),np.max(pics_without_line)
    print(pics_without_line.shape, global_min, global_max)
    ...
    # SAVING
    os.makedirs(f'registered_cropped_top/{scan_num}',exist_ok=True)
    os.makedirs(f'registered_cropped_bottom/{scan_num}',exist_ok=True)
    with open(f'registered_cropped_top/{scan_num}/{scan_num}.pickle', 'wb') as handle:
        pickle.dump(gg[:,:100,:].astype(np.float32), handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'registered_cropped_bottom/{scan_num}/{scan_num}.pickle', 'wb') as handle:
        pickle.dump(gg[:,100:,:].astype(np.float32), handle, protocol=pickle.HIGHEST_PROTOCOL)
    # for i,j in tqdm(enumerate(gg)):
        # image_to_save = (min_max(j)*((2**8)-1)).astype(np.uint8)
        # cv2.imwrite(f'registered_cropped_top/{scan_num}/'+f'frame_test{i}.PNG',image_to_save[:100])
        # cv2.imwrite(f'registered_cropped_bottom/{scan_num}/'+f'frame_test{i}.PNG',image_to_save[100:])
