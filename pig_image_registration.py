...
def opt_flow(slope_arr):
    # mask1 = np.zeros_like(slope_arr[0],dtype=np.float32)
    gg = []
    for i in range(slope_arr.shape[0]-1):
        v, u = optical_flow_tvl1(slope_arr[i], slope_arr[i+1])
...
if __name__ == '__main__':
    ...
    if tot_num_frames-batch<120:
        ...
        arr = np.zeros((120,40,1000,954),dtype=np.float32)
    ...
    X_shape = (arr.shape[0], arr.shape[1], arr.shape[2],arr.shape[3]) # 120x40x1000x954
